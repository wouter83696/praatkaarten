...
// --- PATCH: maak dezelfde "kaartvorm" als op de index ---
// We bouwen een .uCard frame in de stage en stoppen daar de slideTrack in,
// zodat het tekstpaneel eronder er strak onder kan schuiven/overlappen.
function ensureUCardFrame(stage){
  if(!stage) return null;
  const existing = stage.querySelector('.uCard');
  if(existing) return existing;

  const uCard = document.createElement('div');
  uCard.className = 'uCard';

  // Plaats uCard vóór de chevrons, zodat de chevrons altijd erboven liggen.
  const firstChev = stage.querySelector('.chev');
  if(firstChev){
    stage.insertBefore(uCard, firstChev);
  }else{
    stage.appendChild(uCard);
  }
  return uCard;
}

(async function(){
  // Uitleg draait in een iframe (helpModal). Gebruik altijd relatieve paden,
  // zodat het project in elke map / GitHub Pages submap werkt.
  const BASE = ".."; // uitleg/uitleg.html -> project root
  const VERSION = "3.8.4";
  // Standaard set (werkt direct vanuit de zip). Later kun je dit dynamisch maken.
  const SET = "samenwerken";
  ...
  function cardPath(file){
    return `${BASE}/sets/${SET}/cards/${file}`;
  }

  // Verdiepen = Verhelderen (en de "Verdiepen"-kaart verdwijnt uit de uitleg).
  const slides = [
  ...
  let data = {};
  try{
    const res = await fetch(withV(`${BASE}/sets/${SET}/uitleg.json`), { cache:'no-store' });
    data = await res.json();
  }catch(e){
    data = {};
  }
  ...
  // Card frame (zoals index)
  const uCard = ensureUCardFrame(stage);

  const track = document.createElement('div');
  track.className = 'slideTrack';
  (uCard || stage).appendChild(track);
