...
(function(w){
  ...
  function drawBlob(ctx, cx, cy, r, rnd){
    rnd = rnd || Math.random;
    var pts = 8 + Math.floor(rnd()*5); // 8..12
  ...
  function renderSvgLayer(svg, shapes, palette, rnd, lite){
    rnd = rnd || Math.random;
    if(!svg) return;
    // clear
    ...
    for(var n=0;n<maxPick;n++){
      ...
      col = mixWithWhite(col, 0.34 + rnd()*0.16);
  ...
  function renderCanvas(canvas, palette, rnd, lite){
    rnd = rnd || Math.random;
    ...
    for(var i=0;i<blobCount;i++){
      var raw = isDark ? neon[i % neon.length] : ((palette && palette.length) ? palette[i % palette.length] : base);
      ...
      var c = isDark ? mixHex(raw, '#18123c', 0.25) : softenForBg(raw);
      ...
      var cx = (rnd()*1.10 - 0.05) * info.w;
      var cy = (rnd()*1.10 - 0.05) * info.h;
  ...
  PK.indexBackground = {
    render: function(opts){
      ...
      var lite = isFirefox();
      var key = String((opts && opts.cardBase) || '') + '|' + (lite ? 'lite' : 'full');
      ...
      function doRender(assets, seed){
        ...
        var info = renderCanvas(canvas, assets.palette, rnd, lite);
        if(cache){
        ...
        renderSvgLayer(svg, assets.shapes, assets.palette, rnd2, lite);
      }
