...
(function(w){
  ...
  var THEMES = [];
  var THEME_LABELS = {};
  var CARD_BASE = 'sets/samenwerken/cards_rect/';

  // Index achtergrond: 1× genereren en daarna "locken" (geen verandering bij scroll).
  if(PK.indexBackground && PK.indexBackground.render){
    PK.indexBackground.render({ cardBase: CARD_BASE });
  }
...
function closeMenu(){
  ...
  function applySetMeta(setId, meta){
    ...
    CARD_BASE = 'sets/' + encodeURIComponent(setId) + '/cards_rect/';
...
if(PK.createMenuItem){
  ...
  miniImg.src = 'sets/' + encodeURIComponent(setId) + '/cards_rect/' + cardFile;
  miniImg.alt = '';
  ...
  function loadBacks(setId){
    return PK.loadJson(PK.pathForSet ? PK.pathForSet(setId, 'backs.json') : ('sets/' + encodeURIComponent(setId) + '/backs.json')).catch(function(){
      return null;
    });
  }
  ...
  function showError(msg){
    mainCarousel.innerHTML = '<div style="padding:24px;font-family:system-ui;color:#444;">'+msg+'</div>';
  }
  ...
  function applyContrast(mode){
    ...
    // Herteken blobs: dark mode heeft een andere (neon) blob-palette.
    try{ if(PK.indexBackground && PK.indexBackground.render) PK.indexBackground.render({ cardBase: CARD_BASE }); }catch(_e3){}
  }
  if(contrastBtn){
    var savedC = 'light';
...
function openInfo(){
  ...
  function cardPathRect(setId, file){
    return 'sets/' + encodeURIComponent(setId) + '/cards_rect/' + file;
  }
  function cardPathSquare(setId, file){
    return 'sets/' + encodeURIComponent(setId) + '/cards/' + file;
  }
  ...
  function loadAndRender(){
    ...
    var metaUrl = 'sets/' + encodeURIComponent(setId) + '/meta.json';
    var uitlegUrl = 'sets/' + encodeURIComponent(setId) + '/uitleg.json';
  ...
  function setFlipState(cardEl, on){
    ...
    // Safari/iOS fix: force-hide the front face when flipped,
    // so the front text can never bleed through.
    try{
      var front = cardEl.querySelector('.pkFront');
      var back = cardEl.querySelector('.pkBack');
      if(front){
        front.style.display = on ? 'none' : 'block';
      }
      if(back){
  ...
  function renderCards(items){
    ...
    for(var i=0;i<(items||[]).length;i++){
      ...
      var img = w.document.createElement('img');
      img.className = 'bg';
      img.src = PK.withV ? PK.withV(it.bg) : it.bg;
      img.alt = '';
      ...
      if(frontText){
        ...
        q.textContent = safeText(frontText);
        front.appendChild(q);
      }
      ...
      backImg.className = 'bg';
      var backSrc = 'sets/' + encodeURIComponent(CURRENT_SET) + '/cards/' + CURRENT_COVER;
      var backSrcRect = 'sets/' + encodeURIComponent(CURRENT_SET) + '/cards_rect/' + CURRENT_COVER;
      backImg.src = PK.withV ? PK.withV(backSrc) : backSrc;
      ...
      if(backText){
        ...
        bt.textContent = safeText(backText);
        back.appendChild(bt);
      }
