...
// --- PATCH: gebruik vaste <img id="uSlideImg"> in het kaartframe ---
function setUitlegSlideImage(src, alt){
  const img = document.getElementById('uSlideImg');
  if(!img) return;
  img.src = src || '';
  img.alt = alt || '';
}
...
(async function(){
  ...
  setUitlegSlideImage(withV(setPath(s.img || s.src || '')), s.alt || s.title || '');
