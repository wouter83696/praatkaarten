...
const THEMES = ["verkennen","duiden","verbinden","verhelderen","vertragen","bewegen"];
  ...
  const helpItems = [
    { theme:'',            key:'cover',       bg:withV('samenwerken/voorkant.svg') },
    { theme:'Verkennen',   key:'verkennen',   bg:withV('samenwerken/verkennen.svg') },
    { theme:'Duiden',      key:'duiden',      bg:withV('samenwerken/duiden.svg') },
    { theme:'Verbinden',   key:'verbinden',   bg:withV('samenwerken/verbinden.svg') },
    { theme:'Verhelderen', key:'verhelderen', bg:withV('samenwerken/verhelderen.svg') },
    { theme:'Vertragen',   key:'vertragen',   bg:withV('samenwerken/vertragen.svg') },
    { theme:'Bewegen',     key:'bewegen',     bg:withV('samenwerken/bewegen.svg') }
  ];
...
// UI chrome (pijlen + sluiten)
    ...
    for (const theme of THEMES){
      ...
      for (let i=0; i<qs.length; i++){
        out.push({
          ...
          num: i+1,
          q: qs[i],
          bg: withV(`samenwerken/${theme}.svg`),
  ...
  try{
    const q = await fetchJsonFallback('data/questions.json');
...
document.addEventListener('keydown', (e) => {
  ...
  // Mobiel: vaste ✕ knop voor het uitleg-sheet
  const introCloseBtn = document.getElementById('introClose');
  if(introCloseBtn){
    introCloseBtn.addEventListener('click', () => {
      // sluit op dezelfde manier als de info-pill toggelt
      if(uitlegOn) setUitleg(false);
    });
  }

  // Mobiel: sluiten gebeurt via de vaste ✕ knop (#introClose).
...
async function renderMobileIntro(){
  ...
  if(!data){
    try{
      const r = await fetch(withV('data/intro.json'), { cache:'no-store' });
      data = await r.json();
    }catch(e){
      return;
    }
  ...
  const buildCard = (s) => {
    ...
    // Geen close-knop per kaart (voorkomt dubbele ✕). We gebruiken de vaste #introClose.
