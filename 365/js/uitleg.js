(async function(){
  ...
  const slides = [
    { key:"cover",       src:withV(`${BASE}/samenwerken/voorkant.svg`),    alt:"Voorkant" },
    { key:"verkennen",   src:withV(`${BASE}/samenwerken/verkennen.svg`),  alt:"Verkennen" },
    { key:"duiden",      src:withV(`${BASE}/samenwerken/duiden.svg`),     alt:"Duiden" },
    { key:"verbinden",   src:withV(`${BASE}/samenwerken/verbinden.svg`),  alt:"Verbinden" },
    { key:"verhelderen", src:withV(`${BASE}/samenwerken/verhelderen.svg`),alt:"Verhelderen" },
    { key:"vertragen",   src:withV(`${BASE}/samenwerken/vertragen.svg`),  alt:"Vertragen" },
    { key:"bewegen",     src:withV(`${BASE}/samenwerken/bewegen.svg`),    alt:"Bewegen" }
  ];

  let data = {};
  try{
    const res = await fetch(withV(`${BASE}/data/uitleg.json`), { cache:'no-store' });
    data = await res.json();
  }catch(e){
    data = {};
  }
