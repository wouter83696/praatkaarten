...
document.addEventListener('keydown', (e) => {
  ...
  // Mobile: one fixed close button (top-right)
  ...
  function forceCloseIntroSheet(immediate=false){
    if(!introSheet) return;
    document.body.classList.remove('show-intro');
    introSheet.classList.remove('x-scrolling','is-stable');
    introSheet.style.transition = 'none';
    // parkeer ver onder beeld zodat er nooit een 'peek' zichtbaar blijft
    introSheet.style.transform = 'translateY(103%)';
    if(immediate){
      // niets extra; we zetten direct de eindstand
    }
  }
