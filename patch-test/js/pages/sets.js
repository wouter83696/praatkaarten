...
(function(w){
  ...
  function buildHeroSlide(args){
    ...
    if(!args.placeholder){
      ...
      img.className = 'bg';
      var rectSrc = 'sets/' + encodeURIComponent(args.setId) + '/cards_rect/' + args.file;
      var fullSrc = 'sets/' + encodeURIComponent(args.setId) + '/cards/' + args.file;
      img.src = PK.withV ? PK.withV(fullSrc) : fullSrc;
      img.onerror = function(){
        if(this.getAttribute('data-fallback') === '1') return;
        this.setAttribute('data-fallback','1');
        this.src = PK.withV ? PK.withV(rectSrc) : rectSrc;
      };
      img.alt = '';
  ...
  function renderHero(idx, activeSetId, activeMeta){
    ...
    for(var o=0;o<ordered.length;o++){
      ...
      carousel.appendChild(buildHeroSlide({
        setId: id,
        file: file,
        href: 'kaarten.html?set=' + encodeURIComponent(id),
        label: label
      }));
      count++;
    }
  ...
  function applyBackground(idx){
    ...
    var cardBase = 'sets/' + encodeURIComponent(baseId) + '/cards_rect/';
  ...
  function init(){
    ...
    PK.loadJson(PK.PATHS ? PK.PATHS.setsIndex : 'sets/index.json').then(function(idx){
      idx = idx || {};
      ...
      return PK.loadJson('sets/' + encodeURIComponent(setId) + '/meta.json').then(function(meta){
        return { idx: idx, setId: setId, meta: meta || {} };
      }, function(){
        return { idx: idx, setId: setId, meta: {} };
      });
    }).then(function(state){
      ...
      if(menuList){
        menuList.addEventListener('click', function(e){
          var btn = e.target && (e.target.closest ? e.target.closest('button[data-set],a[data-set]') : null);
          if(!btn) return;
          var targetSet = trim(btn.getAttribute('data-set') || '');
          if(!targetSet) return;
          try{ if(menuApi && menuApi.close) menuApi.close(); }catch(_eClose){}
          w.location.href = 'kaarten.html?set=' + encodeURIComponent(targetSet);
        });
      }
      ...
      if(dots) dots.innerHTML = '';
      if(grid){
        grid.innerHTML = '<div style="padding:12px 0;font-family:system-ui;color:#444;">Geen kaartensets gevonden.</div>';
      }
    });
  }
