...
export function validateUiConfig(cfg){
  ...
  var ok = true;
  var allowed = { menu:1, sheet:1, index:1, cardsIndex:1, vars:1, themeCss:1 };
  for(var k in cfg){
  ...
  if(cfg.cardsIndex){
    if(cfg.cardsIndex.background !== undefined && (!cfg.cardsIndex.background || typeof cfg.cardsIndex.background !== 'object' || Array.isArray(cfg.cardsIndex.background))){ warn('cardsIndex.background moet object zijn'); ok = false; }
  }
  if(cfg.index){
