...
const THEMES = ["verkennen","duiden","verbinden","verdiepen","vertragen","bewegen"];
  ...
  function openInline(item){
    ...
    requestAnimationFrame(() => {
      // Wacht 1 frame zodat display:block echt toegepast is, dan scroll robust.
      const y = window.scrollY + inlineViewer.getBoundingClientRect().top;
      const offset = 16; // kleine ademruimte boven viewer
      try{ window.scrollTo({ top: Math.max(0, y - offset), behavior: 'smooth' }); }
      catch(e){ window.scrollTo(0, Math.max(0, y - offset)); }
    });
  }
