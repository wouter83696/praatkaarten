...
(async function(){
  // Forward touch/pointer gestures to parent (index) so swipe-down close works
  // even when the user starts dragging on iframe content.
  (function(){
    try{
      ...
      function send(phase, x, y){
        window.parent.postMessage({ type: 'PK_INTRO_GESTURE', phase, x, y, origin: ORIGIN }, ORIGIN);
      }
      ...
      window.addEventListener('touchstart', (e)=>{
        ...
        if(!t) return;
        active = true;
        send('start', t.clientX, t.clientY);
