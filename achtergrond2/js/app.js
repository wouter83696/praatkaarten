...
// ===============================
// Grid-achtergrond (vormen) – automatisch per kaartenset
// - Leest optioneel sets/<set>/palette.json (aanrader)
// - Anders: pakt kleuren uit UI (pills/tekst) of fallback palette
// - randomMode: 'stable' (vast per set) of 'fresh' (elke reload anders)
// ===============================
...
function _isNearWhite(rgb){
  const m = String(rgb||'').match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/i);
  if(!m) return false;
  const r=+m[1], g=+m[2], b=+m[3];
  return (r>235 && g>235 && b>235);
}
function _cleanColor(val){
  const v = String(val||'').trim();
  if(!v) return '';
  if(v==='transparent' || v==='none' || v==='rgba(0, 0, 0, 0)') return '';
  return v;
}
function _pickColorsFromUI(){
  const c = new Set();
  try{
    const root = getComputedStyle(document.documentElement);
    ['--accent','--primary','--theme','--brand'].forEach(k=>{
      const v=_cleanColor(root.getPropertyValue(k));
      if(v) c.add(v);
    });
  }catch(e){}

  const pills = document.querySelectorAll('.pill, .theme-pill, [data-theme], .tag');
  pills.forEach(el=>{
    const cs=getComputedStyle(el);
    const bg=_cleanColor(cs.backgroundColor);
    const fg=_cleanColor(cs.color);
    if(bg && !_isNearWhite(bg)) c.add(bg);
    if(fg && !_isNearWhite(fg)) c.add(fg);
  });

  // Kaarten zijn wit, dus pak vooral tekstkleur/borderkleur waar aanwezig
  const cards = document.querySelectorAll('.card, .kaart, .card-item, [data-card], .grid > *');
  cards.forEach(el=>{
    const cs=getComputedStyle(el);
    const bc=_cleanColor(cs.borderColor);
    const fg=_cleanColor(cs.color);
    if(bc && !_isNearWhite(bc)) c.add(bc);
    if(fg && !_isNearWhite(fg)) c.add(fg);
  });

  const arr = [...c].filter(Boolean);
  // Fallback (past bij jouw huidige stijl)
  if(arr.length < 2){
    return ['rgba(90,135,210,1)','rgba(90,190,155,1)','rgba(235,210,140,1)','rgba(182,167,230,1)'];
  }
  return arr.slice(0,5);
}
async function _pickPaletteForSet(){
  // Optioneel: sets/<set>/palette.json  -> {"palette": ["#...", ...]}
  try{
    const p = await fetchJsonFallback(withV(setPath('palette.json')));
...
async function applyBackgroundForSet(setId, opts={}){
  const randomMode = opts.randomMode || 'stable'; // 'stable' | 'fresh'
  const bg = _ensureBgLayer();
  bg.innerHTML = '';

  let palette = await _pickPaletteForSet();
  if(!palette) palette = _pickColorsFromUI();

  const seed = (randomMode==='fresh') ? Math.floor(Math.random()*1e9) : _hashString(String(setId||'default'));
  const rnd = _mulberry32(seed);

  const pick = () => palette[Math.floor(rnd()*palette.length)];

  // Vormtaal: grafische vlakken (geen zweverige blobs)
  const shapes = [
    {
      color: pick(),
      left: '-12%',
      top: `${Math.floor(rnd()*18)}%`,
      width: '70%',
      height: '62%',
      radius: '140px',
      blur: 10,
  ...
  shapes.forEach(s=>bg.appendChild(_makeShape(s)));
}
window.applyBackgroundForSet = applyBackgroundForSet;
// Start achtergrond meteen (stable per set)
try{
  requestAnimationFrame(()=>{ applyBackgroundForSet(ACTIVE_SET,{randomMode:'stable'}).catch(()=>{}); });
}catch(e){}
