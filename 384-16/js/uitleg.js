...
// --- PATCH: force SVG into card frame (.uCard) ---
function _wrapStageImageInCard(){
  const stage = document.getElementById('stage');
  if(!stage) return;
  const img = stage.querySelector('img');
  if(!img) return;
  if(img.closest('.uCard')) return;

  const uCard = document.createElement('div');
  uCard.className = 'uCard';
  img.parentNode.insertBefore(uCard, img);
  uCard.appendChild(img);
}
