...
// UI chrome (pijlen + sluiten)
  ...
  try{
    ...
    if(!Array.isArray(data)) throw new Error("questions.json is geen array");
    return data;
  }catch(err){
    console.error("Kon data/questions.json niet laden.", err);
    showToast("Kan vragenlijst niet laden. Open via GitHub Pages of een lokale server.", 5000);
    return [];
  }
}
